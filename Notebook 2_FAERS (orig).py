...
df_converted2 = df_converted.select_dtypes(exclude='object') \
                            .drop(['primaryid','caseid','caseversion','event_dt','mfr_dt','init_fda_dt','fda_dt','wt', \
                                    'rept_dt','last_case_version','val_vbm','pt_consolidated','start_dt','end_dt'], axis=1)

#cols =  df_converted.columns[~df_converted.columns.str.endswith('dt')].tolist()

# COMMAND ----------

df_converted2.dtypes
...
#df_converted2.select_dtypes(exclude='object').isna().any()
...
# X = input
X = df_converted2.drop("outc_cod_DE" ,axis= 1)

print(X)

# y = output
#y = df4['outc_cod_DE']
y = df_converted2['outc_cod_DE']
...
# MAGIC * True Negative (TN) = 452 patients were predicted to not die (as a result of being given corticosteroids), and actually did not die.  The algorithm predicted it correctly 
# MAGIC * False Positive (FP) = 0 patients were predicted to not die, but actually did die.  
# MAGIC * False Negative (FN) = 59 patients were predicted to die, but actually did not die.
# MAGIC * True Positive (TP) = 0 patients were predicted to die, and actually did die.
# MAGIC 
# MAGIC The goal is to keep TN and TP high.  We can certainly see that the True Negatives (452) and True Positives (0) are not significantly higher than the false predictions.  
# MAGIC 
# MAGIC ##### _Which metric is more costly?_  
# MAGIC 
# MAGIC (FP) If the model predicted that >100 patients would not die (as a result of being given corticosteroids), but actually did die, this could be very costly to a hospital.  This should be minimized as much as possible.
# MAGIC 
# MAGIC (FN) If the model predicted that >100 patients would die (as a result of being given corticosteroids), but actually did not die, this would be less costly to a hospital.
# MAGIC 
...
def display_dict_models(dict_models, sort_by='test_score'):
    ...
    for ii in range(0,len(cls)):
        ...
        df_.loc[ii, 'precision'] = test_s[ii]
        df_.loc[ii, 'recall'] = test_s[ii]
        df_.loc[ii, 'f1'] = f1_s[ii]
        df_.loc[ii, 'fbeta'] = f1_s[ii]
